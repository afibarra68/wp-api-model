...
import type { Campaign, CampaignMapeo, CampaignMetricas, CampaignSegmento } from '../types/entities';
...
const FIELDS = `id, nombre_campana, plantilla_id, integration_id, segmento, mapeo_variables,
...
export async function createCampaign(input: {
  ...
  const { rows } = await getPool().query<Row>(
    `INSERT INTO campaigns (nombre_campana, plantilla_id, integration_id, segmento, mapeo_variables)
...
export async function finalizeCampaignIfDone(campaignId: string): Promise<void> {
  await getPool().query(
    `UPDATE campaigns SET estado = 'finalizada', fecha_finalizacion = NOW()
     ...
     AND NOT EXISTS (
       SELECT 1 FROM message_logs WHERE campana_id = $1 AND estado_actual = 'encolado'
     )`,
    [campaignId],
  );
}
