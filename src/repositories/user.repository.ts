...
const FIELDS = 'id, nombre, email, password_hash, rol, activo, ultimo_login, created_at, updated_at';
...
export async function findUserByEmail(email: string, withPassword = false): Promise<User | null> {
  ...
  if (u && !withPassword) delete u.passwordHash;
  return u;
}
...
export async function findAllUsers(): Promise<User[]> {
  const { rows } = await getPool().query<Row>(
    `SELECT id, nombre, email, rol, activo, ultimo_login, created_at, updated_at FROM users ORDER BY created_at DESC`,
  );
...
export async function createUser(input: {
  ...
  const { rows } = await getPool().query<Row>(
    `INSERT INTO users (nombre, email, password_hash, rol) VALUES ($1,$2,$3,$4)
     RETURNING id, nombre, email, rol, activo, ultimo_login, created_at, updated_at`,
    [input.nombre, input.email.toLowerCase(), input.passwordHash, input.rol ?? 'agente'],
  );
...
export async function updateUser(
  id: string,
  patch: Partial<{ nombre: string; rol: Role; activo: boolean; ultimoLogin: Date; passwordHash: string }>,
): Promise<User | null> {
  const sets: string[] = [];
  const vals: unknown[] = [id];
  let i = 2;
  if (patch.nombre !== undefined) { sets.push(`nombre = $${i++}`); vals.push(patch.nombre); }
  if (patch.rol !== undefined) { sets.push(`rol = $${i++}`); vals.push(patch.rol); }
  if (patch.activo !== undefined) { sets.push(`activo = $${i++}`); vals.push(patch.activo); }
  if (patch.ultimoLogin !== undefined) { sets.push(`ultimo_login = $${i++}`); vals.push(patch.ultimoLogin); }
